    ...
    public partial class WebForm3 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                List.DataSource = Con.GetApplications();
                List.DataBind();
                DropDownList5.DataSource = Con.GetStatuses();
                DropDownList5.DataBind();
                GridView7.DataSource = Con.GetExpired();
                GridView7.DataBind();
            }
        }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            string construct = DropDownList1.Text;
            Con = new Models.DBQueries();
            var LoginID = Session["loginID"];
            Con.SetApplication(app,name, surname, otch, address, type, construct, LoginID.ToString());
            List.DataSource = Con.GetApplications();
            List.DataBind();
        }
        ...
        {
            DataTable dt = Con.GetExpiredDate();
            foreach (DataRow row in dt.Rows)
            {
                if (Convert.ToDateTime(e.Day.Date) == Convert.ToDateTime(row[0]))
